function FSS(container, output){
	...
	function() {
		...
		b(); (function() {
			var b, d;
			for (b = B.lights.length - 1; 0 <= b; b--) d = B.lights[b], B.remove(d);
			D.clear();
		...
		return {
			setLight: function(ambient, diffuse) {
				for(var i=0; i<B.lights.length; ++i) {
					B.lights[i].ambient.set(ambient);
					B.lights[i].diffuse.set(diffuse);
				}
			},
			setMesh: function(ambient, diffuse) {
				for(var i=0; i<B.meshes.length; ++i) {
					console.log(B.meshes[i]);
					B.meshes[i].material.ambient.set(ambient);
					B.meshes[i].material.diffuse.set(diffuse);
				}
			}
		};
